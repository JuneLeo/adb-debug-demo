...
public class FileManager {
    ...
    public static File getTempDexFile() {
        ...
        if (Log.isLoggable(Logging.LOG_TAG, Log.VERBOSE)) {
            Log.v(Logging.LOG_TAG, "Writing new dex file: " + file);
        }
    ...
    public static boolean writeRawBytes(@NonNull File destination, @NonNull byte[] bytes) {
        try {
            ...
            Log.wtf(
                    Logging.LOG_TAG,
                    "Failed to write file, clean project and rebuild " + destination,
                    ioe);
    ...
    public static boolean extractZip(@NonNull File destination, @NonNull byte[] zipBytes) {
        if (USE_EXTRACTED_RESOURCES) {
            ...
            try {
                ...
                for (ZipEntry entry = zipInputStream.getNextEntry();
                    ...
                    if (!entry.isDirectory()) {
                        ...
                        if (parent != null && !parent.exists()) {
                            ...
                            if (!created) {
                                Log.e(Logging.LOG_TAG, "Failed to create directory " + dest);
                                return false;
                            }
                        }
                ...
                Log.e(
                        Logging.LOG_TAG,
                        "Failed to extract zip contents into directory " + destination,
                        ioe);
                return false;
            } finally {
                try {
        ...
        if (USE_EXTRACTED_RESOURCES) {
            try {
                ...
                if (Log.isLoggable(Logging.LOG_TAG, Log.ERROR)) {
                    Log.e(Logging.LOG_TAG, "Couldn't look up message digest", e);
                }
            } catch (IOException ioe) {
                if (Log.isLoggable(Logging.LOG_TAG, Log.ERROR)) {
                    Log.e(Logging.LOG_TAG, "Failed to read file " + file, ioe);
                }
            } catch (Throwable t) {
                if (Log.isLoggable(Logging.LOG_TAG, Log.ERROR)) {
                    Log.e(Logging.LOG_TAG, "Unexpected checksum exception", t);
                }
            }
        }
        return null;
